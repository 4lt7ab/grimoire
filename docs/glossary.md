...
**`entry_idx`.** The filterable/sortable metadata sidecar. One row per indexed entry. Columns: `uniq_id` (PK), `uniq_ref`, `group_ref`, `owner_ref`, and five symmetric `ordinal_1`..`ordinal_5` columns (BLOB-affinity — any storage class accepted). Written by `index(uniq_id, ref=..., group=..., owner=..., ord=...)`; cleaned by the entry-delete trigger.
...
**Index.** `Grimoire.index(uniq_id, *, ref, group, owner, ord, match, search)`. The combined sidecar writer. PUT-style: each supplied kwarg wholesale-replaces the corresponding sidecar row; omitted kwargs leave that side alone. `ord` is a 5-tuple addressing `ordinal_1`..`ordinal_5`; in-tuple `None` writes NULL to that column.
...
**`owner_ref`.** Non-unique TEXT column on `entry_idx` for tagging entries with an owner (e.g. a user, account, or principal id). Backed by a partial index (`WHERE owner_ref IS NOT NULL`), so `query(Filters(equals={"owner_ref": [...]}))` seeks rather than scans. Like `group_ref` and unlike `uniq_ref`, it enforces no uniqueness — any number of rows may share an `owner_ref`. It sits alongside `group_ref` as a second independent grouping axis, so an entry can carry both an owner and an orthogonal grouping key. Written by `index(uniq_id, owner=...)`.
